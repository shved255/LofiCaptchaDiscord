...
public class CommandListener extends ListenerAdapter {
   ...
   public void onSlashCommandInteraction(SlashCommandInteractionEvent event) {
         ...
         String embedTitle = Main.getInstance().getConfig().getString("Discord.Title");
         String embedDescription =  Main.getInstance().getConfig().getString("Discord.Description");
         Color embedColor = Color.decode(Main.getInstance().getConfig().getString("Discord.Color"));
         ...
         event.reply(Main.getInstance().getConfig().getString("Discord.SuccessSendPacket")).setEphemeral(true).queue();
      }
   ...
   public void onButtonInteraction(ButtonInteractionEvent event) {
      if(event.getComponentId().equals("open-form")) {
         ...
         if (goyda) {
            ...
            Modal modal = Modal.create("verif-modal", "Верификация").addComponents(new LayoutComponent[]{ActionRow.of(new ItemComponent[]{nick1})}).build();
